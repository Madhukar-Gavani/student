...
void Adafruit_L3GD20::calibrateGyro(uint8_t times)
{
    ...
    sigmaX += data.x  * data.x ;
    sigmaY += data.y  * data.y ;
    sigmaZ += data.z  * data.z ;
  ...
  delay(5);
    ...
    threshold.x = sqrt((sigmaX / 50) - (delta.x *delta.x));
    threshold.y= sqrt((sigmaY / 50) - (delta.y * delta.y));
    threshold.z = sqrt((sigmaZ / 50) - (delta.z *delta.z));
...
void Adafruit_L3GD20::readGyro()
{
   ...
   if(userCalibrate)
   {
       gyro.x = data.x - delta.x;
       gyro.y = data.y - delta.y;
       gyro.z = data.z - delta.z;
    }
     else
    {
       gyro.x = data.x;
