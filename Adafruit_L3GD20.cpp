...
void Adafruit_L3GD20::calibrateGyro(uint8_t times)
{
  ...
  delay(5);
    ...
    threshold.x = sqrt((sigmaX /times) - pow(delta.x ,2));
    threshold.y= sqrt((sigmaY / times) - pow(delta.y ,2));
    threshold.z = sqrt((sigmaZ / times) - pow(delta.z ,2));
...
void Adafruit_L3GD20::setThreshold(uint8_t multiples)
{
    if(multiples > 0)
    {
        if(!userCalibrate)
        ...
        threshold_vectors.x = threshold.x * multiples;
        threshold_vectors.y = threshold.y * multiples; 
        threshold_vectors.z = threshold.z * multiples;
    }
    else 
    {
        threshold_vectors.x = 0;
        threshold_vectors.y = 0; 
        threshold_vectors.z = 0; 
    }

  actualThreshold = multiples;

}  

void Adafruit_L3GD20::readGyro()
{
   read();
   if(userCalibrate)
   {  
       gyro.x = (data.x - delta.x)*L3GD20_SENSITIVITY_2000DPS;
       gyro.y = (data.y - delta.y)*L3GD20_SENSITIVITY_2000DPS;
       gyro.z = (data.z - delta.z)*L3GD20_SENSITIVITY_2000DPS;
    }
     else
    {
       gyro.x = data.x*L3GD20_SENSITIVITY_2000DPS;
       gyro.y = data.y*L3GD20_SENSITIVITY_2000DPS;
       gyro.z = data.z*L3GD20_SENSITIVITY_2000DPS;
    }
  if(actualThreshold)
  {
     if (abs(gyro.x) < threshold_vectors.x ) gyro.x = 0;
    if (abs(gyro.y) < threshold_vectors.y ) gyro.y = 0;
    if (abs(gyro.z) < threshold_vectors.z ) gyro.z = 0;
  }
}
